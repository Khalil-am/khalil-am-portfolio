...
The interesting product problem was never _building the forms_. It was this:
...
A policy that lives in a binder is a suggestion. A policy that lives in the workflow engine is _enforced_ — and because every action passes through a defined gate, taken by a named role, auditability is not a reporting feature bolted on at the end. It is the shape of the system.

I have applied the same conviction elsewhere — in [StoryIQ's config-driven approval workflow](/blog/storyiq-signal-to-pass) and in [encoding regulatory rules as software for Wathiq](/blog/wathiq-e-invoicing-platform). IMDAD is the largest expression of it so far: an entire procurement rulebook, running as a platform.
