...
summary: "KAM CLI is my open-source, developer-first command-line toolkit — it automates repetitive engineering tasks and productivity shortcuts so I ship faster every day."
image: "/kam-cli.png"
...
- First-class integration with [KAM AI](/blog/KAM-AI) and [BC Automations](/blog/BC-Automations), so the CLI becomes the "hands" for higher-level agents.

At its core, KAM CLI is a bet that **the terminal isn't going anywhere** — it's just getting smarter. This toolkit is my way of making sure my terminal keeps up with [the rest of my stack](/projects).
...
KAM CLI is a developer-first command-line toolkit — a single, consistent entrypoint for the workflows I run every day: repo scaffolding, environment bootstrapping, code and docs operations, commit hygiene, and project utilities.

### Is KAM CLI open source?

Yes. It lives publicly on [GitHub](https://github.com/Khalil-am/kam-cli), because my workflow isn't a trade secret and the parts that generalize should be shared.

### Can AI agents use KAM CLI?

Yes — humans and agents share the same interface. Whatever works for me in a terminal also works for Claude Code inside an automation.
